...
import goldBuying2 from "@/assets/images/services/gold-buying2.jpg";
    ...
    <>
      ...
      <section className="relative py-36 md:py-48 overflow-hidden">
        <div className="absolute inset-0">
          <img src={goldBuying2} className="w-full h-full object-cover" alt="Gold services" loading="eager" />
          <div className="absolute inset-0 bg-gradient-to-t from-background via-background/80 to-background/60" />
        </div>
        <div className="relative z-10 max-w-7xl mx-auto px-6">
          <AnimatedSection>
            <div className="max-w-2xl">
              <span className="label-gold mb-6 block w-fit">Our Services</span>
      ...
      {/* Services Detail — alternating full-bleed */}
          ...
          {services.map((service, i) => (
            <div key={i} className={`grid lg:grid-cols-2 gap-16 items-center`}>
              <AnimatedSection direction={i % 2 === 0 ? "left" : "right"} className={i % 2 !== 0 ? "lg:order-2" : ""}>
                <motion.div whileHover={{ scale: 1.02 }} transition={{ duration: 0.4 }} className="relative overflow-hidden gold-border">
                  <img src={service.img} className="w-full h-[420px] object-cover" alt={service.title} loading="lazy" />
                  <div className="absolute inset-0 bg-gradient-to-t from-crimson/50 to-transparent" />
                </motion.div>
              </AnimatedSection>
